...
def dam_cascade_mesh(mat, name='dam_breach_cascade_nocol', sections=None):
    ...
    if sections is None:
        # Leave the flyable reservoir through the opening, then stay downstream
        # of the lower buttress to the floor-side runtime jet at Blender Y=125.
        sections = ((375, -64, 71, 0), (368, 125, 66, -5), (335, 130, 70, 4),
                    (285, 131, 86, -7), (235, 129, 98, 6), (145, 128, 94, -3),
                    (20, 125, 108, 0))
    vertices = []
...
def build_dam(parent=None, collapse=False):
    ...
    # Match the flyable rear reservoir: authored bounds z=90..95, level=75.
    # The map places this Blender plane at world Y=225 and Z=270..285.
    objects.append(box('dam_reservoir_surface_nocol', (0, -52, 375), (900, 25, 1.8), water, parent))
    backwater = dam_cascade_mesh(water, 'dam_reservoir_body_nocol', (
        (20, -67, 79, -4), (67, -66, 88, 3), (111, -67, 82, -6),
        (157, -65, 98, 5), (205, -66, 93, -3), (250, -66, 111, 7),
        (301, -65, 105, -6), (335, -67, 121, 2), (360, -66, 133, 5),
        (375, -65, 140, 0),
    ))
    if parent is not None:
    ...
    for index, x in enumerate((-59, -30, 12, 53)):
        ...
        stripe = dam_cascade_mesh(foam, f'dam_reservoir_rivulet_{index}_nocol', (
            (28, -62, 2.5, x - 6), (133, -61, 4, x + 3),
            (257, -62, 2, x - 4), (335, -61, 3.5, x + 7),
            (371, -61, 2, x),
        ))
        if parent is not None:

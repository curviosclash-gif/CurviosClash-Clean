...
## Ponytail-lite boundary

Apply the [Ponytail-lite baseline](../adaptive-model-routing/SKILL.md#ponytail-lite-baseline) to orchestration, script reuse, dependencies, and repair scope. It may reduce duplicated work or select the smallest failing row for repair, but it may never remove required visual jobs, animation rows, direction checks, deterministic validation, independent QA, packaging fields, or cleanup safeguards in this skill.
...
- Use `economy_scout` for brand discovery and other read-only evidence gathering.
- Use `balanced_worker` for each bounded image-generation job and deterministic test task. Use `economy_scout` for isolated visual QA when it can inspect the required images.
- Use `expert_reviewer` only for persistent identity, direction-semantics, assembly, or data-loss ambiguity after the normal worker loop fails. Use the parent/default model only for orchestration or when the smaller roles are unavailable.
- Spawn workers with `fork_turns: none` and the minimum prompt, paths, references, and acceptance evidence needed for that one job.

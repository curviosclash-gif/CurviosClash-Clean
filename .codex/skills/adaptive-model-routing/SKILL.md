...
- Use `gpt-6-luna` at low effort for bounded triage, extraction, or a small edit with an objective check. Medium effort may fit a coordinated change with a clear brief. For exact derived answers across files, verify with a deterministic tool or use `gpt-6-sol`. Keep judgment-heavy integration with Root.

...
export class HangarVehicleAssembly {
    ...
    _material(color, options = {}) {
        ...
        material.userData.hangarBaseEmissive = options.emissive ?? 0x000000;
        material.userData.hangarBaseEmissiveIntensity = options.emissiveIntensity ?? 0;
    ...
    _createPartNode(part, { ghost = false, valid = true } = {}) {
        ...
        const structureMaterial = this._material(ghost ? color : 0x243750, {
            roughness: 0.42,
            metalness: 0.82,
            transparent: ghost,
            opacity: ghost ? 0.32 : 1,
            emissive: ghost ? color : 0x050b12,
            emissiveIntensity: ghost ? 0.4 : 0.08,
        });
        const accentMaterial = this._material(color, {
            roughness: 0.2,
            metalness: 0.5,
            transparent: ghost,
            opacity: ghost ? 0.62 : 1,
            emissive: color,
            emissiveIntensity: ghost ? 0.8 : 0.52,
        });
        const root = new THREE.Group();
        const variant = String(part.appearance?.variant || '');
        const add = (geometry, position = [0, 0, 0], scale = [1, 1, 1], rotation = [0, 0, 0], meshMaterial = material) => {
        ...
        if (part.visual === 'stone') {
            ...
            if (variant === 'swift') {
                ...
                add(this._geometry('part-core-swift-fin', () => new THREE.ConeGeometry(0.08, 0.34, 3)), [-0.25, -0.03, 0], [1, 1, 1], [0, 0, Math.PI / 2], accentMaterial);
                add(this._geometry('part-core-swift-fin', () => new THREE.ConeGeometry(0.08, 0.34, 3)), [0.25, -0.03, 0], [1, 1, 1], [0, 0, -Math.PI / 2], accentMaterial);
            } else if (variant === 'reactor') {
                add(this._geometry('part-core-reactor', () => new THREE.SphereGeometry(0.24, 14, 10)), [0, 0, 0], [tierScale, tierScale, tierScale], [0, 0, 0], accentMaterial);
                add(this._geometry('part-core-reactor-ring', () => new THREE.TorusGeometry(0.43, 0.045, 8, 24)), [0, 0, 0], [1, 1, 1], [Math.PI / 2, 0, 0], structureMaterial);
                add(this._geometry('part-core-reactor-ring', () => new THREE.TorusGeometry(0.43, 0.045, 8, 24)), [0, 0, 0], [1, 1, 1], [0, Math.PI / 2, 0], structureMaterial);
                ...
                add(this._geometry('part-core-bastion-armor', () => new THREE.BoxGeometry(0.76, 0.12, 0.24)), [0, -0.18, 0], [1, 1, 1], [0, 0, 0], structureMaterial);
                add(this._geometry('part-core-bastion-armor', () => new THREE.BoxGeometry(0.76, 0.12, 0.24)), [0, -0.18, 0], [1, 1, 1], [0, Math.PI / 2, 0], structureMaterial);
            } else {
                add(this._geometry('part-core-aegis', () => new THREE.OctahedronGeometry(0.34, 1)), [0, 0, 0], [tierScale, tierScale, tierScale]);
                add(this._geometry('part-core-aegis-ring', () => new THREE.TorusGeometry(0.38, 0.035, 8, 24)), [0, 0, 0], [1, 1, 1], [Math.PI / 2, 0, 0], accentMaterial);
            }
            ...
            add(this._geometry('part-nose-mount', () => new THREE.CylinderGeometry(0.2, 0.26, 0.14, 12)), [0, -0.3, 0], [1, 1, 1], [0, 0, 0], structureMaterial);
            if (variant === 'razor') {
                ...
                add(this._geometry('part-nose-bulwark-plate', () => new THREE.BoxGeometry(0.68, 0.13, 0.48)), [0, -0.16, 0], [1, 1, 1], [0, 0, 0], structureMaterial);
                add(this._geometry('part-nose-bulwark-brow', () => new THREE.BoxGeometry(0.5, 0.12, 0.18)), [0, 0.16, 0.06], [1, 1, 1], [0, 0, 0], accentMaterial);
                ...
                add(this._geometry('part-nose-phantom-prong', () => new THREE.ConeGeometry(0.1, 0.78, 8)), [0.17, 0.04, 0], [tierScale, 1.2 * tierScale, tierScale]);
                add(this._geometry('part-nose-phantom-bridge', () => new THREE.BoxGeometry(0.46, 0.12, 0.22)), [0, -0.25, 0], [1, 1, 1], [0, 0, 0], structureMaterial);
            } else {
            ...
            add(this._geometry('part-wing-mount', () => new THREE.BoxGeometry(0.36, 0.13, 0.3)), [-0.3, -0.04, 0], [1, 1, 1], [0, 0, 0], structureMaterial);
            if (variant === 'kestrel') {
                ...
                add(this._geometry('part-wing-kestrel-tail', () => new THREE.BoxGeometry(0.42, 0.05, 0.18)), [-0.25, 0.03, 0.19], [1, 1, 1], [0, -0.35, 0], accentMaterial);
                ...
                add(this._geometry('part-wing-guardian-rail', () => new THREE.BoxGeometry(0.82, 0.06, 0.08)), [0, 0.13, -0.2], [1, 1, 1], [0, 0, 0], accentMaterial);
                ...
                add(this._geometry('part-wing-specter-blade', () => new THREE.BoxGeometry(0.92, 0.045, 0.16)), [0.04, 0, 0.15], [tierScale, 1, tierScale], [0, -0.18, 0]);
                add(this._geometry('part-wing-specter-bridge', () => new THREE.BoxGeometry(0.34, 0.08, 0.44)), [-0.3, 0, 0], [1, 1, 1], [0, 0, 0], structureMaterial);
            } else {
            ...
            add(this._geometry('part-engine-mount', () => new THREE.BoxGeometry(0.4, 0.16, 0.28)), [0, 0.28, 0], [1, 1, 1], [0, 0, 0], structureMaterial);
            if (variant === 'eco') {
                add(this._geometry('part-engine-eco', () => new THREE.CylinderGeometry(0.11, 0.15, 0.62, 12)), [-0.14, 0, 0], [tierScale, tierScale, tierScale]);
                add(this._geometry('part-engine-eco', () => new THREE.CylinderGeometry(0.11, 0.15, 0.62, 12)), [0.14, 0, 0], [tierScale, tierScale, tierScale]);
                add(this._geometry('part-engine-eco-bridge', () => new THREE.BoxGeometry(0.42, 0.22, 0.18)), [0, 0.08, 0], [1, 1, 1], [0, 0, 0], structureMaterial);
                ...
                add(this._geometry('part-engine-nova-ring', () => new THREE.TorusGeometry(0.36, 0.06, 8, 20)), [0, -0.36, 0], [1, 1, 1], [Math.PI / 2, 0, 0], structureMaterial);
                add(this._geometry('part-engine-nova-core', () => new THREE.SphereGeometry(0.16, 12, 8)), [0, -0.42, 0], [1, 1, 1], [0, 0, 0], accentMaterial);
            } else {
                add(this._geometry('part-engine-ion', () => new THREE.CylinderGeometry(0.2, 0.25, 0.54, 16)), [0, 0, 0], [tierScale, tierScale, tierScale]);
                add(this._geometry('part-engine-ion-nozzle', () => new THREE.TorusGeometry(0.21, 0.045, 8, 18)), [0, -0.28, 0], [1, 1, 1], [Math.PI / 2, 0, 0], accentMaterial);
            }
            ...
            add(this._geometry('part-utility-mount', () => new THREE.CylinderGeometry(0.32, 0.38, 0.12, 14)), [0, -0.16, 0], [1, 1, 1], [0, 0, 0], structureMaterial);
            if (variant === 'flux') {
                ...
                add(this._geometry('part-utility-flux-orb', () => new THREE.SphereGeometry(0.12, 12, 8)), [0, 0, 0], [1, 1, 1], [0, 0, 0], accentMaterial);
                ...
                add(this._geometry('part-utility-shield-base', () => new THREE.CylinderGeometry(0.4, 0.4, 0.08, 16)), [0, -0.1, 0], [1, 1, 1], [0, 0, 0], structureMaterial);
                ...
                add(this._geometry('part-utility-pulse-orb', () => new THREE.SphereGeometry(0.11, 12, 8)), [0, 0, 0], [1, 1, 1], [0, 0, 0], accentMaterial);
            }
    ...
    setSelectedSlot(slotId) {
        ...
        for (const [id, node] of this.partNodes.entries()) {
            ...
            node.traverse((child) => {
                ...
                child.material.emissive.setHex(selected ? 0x36a9ff : child.material.userData.hangarBaseEmissive);
                child.material.emissiveIntensity = selected ? 0.85 : child.material.userData.hangarBaseEmissiveIntensity;
            });
        }
    }
    ...
    setComparison(compareBuild) {
        ...
        for (const [slotId, node] of this.partNodes) {
            ...
            node.traverse((child) => {
                ...
                child.material.emissive.setHex(changed ? 0xf59e0b : child.material.userData.hangarBaseEmissive);
                child.material.emissiveIntensity = changed ? 0.72 : child.material.userData.hangarBaseEmissiveIntensity;
            });
        }
    }

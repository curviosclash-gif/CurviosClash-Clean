...
function resolveWinnerLabel(lastRoundMetrics, players) {
    ...
    if (!Number.isFinite(winnerIndex) || winnerIndex < 0) {
...
function resolveObjectiveLabel(reason) {
    ...
    if (normalized === 'PARCOURS_COMPLETE') {
        return 'Parcours abgeschlossen';
    }
...
function buildParcoursBlock(lastRoundMetrics) {
    if (!lastRoundMetrics || lastRoundMetrics.parcoursCompleted !== true) {
        return null;
    }
    return {
        id: 'parcours',
...
function buildEndlessParcoursBlock(outcome) {
    ...
    return {
        ...
        rows: [
            ...
            { key: 'elite-kills', label: 'Anfuehrer', value: String(Math.max(0, Math.floor(normalizeNumber(summary.eliteKills, 0)))) },
            { key: 'gates', label: 'Tore', value: String(Math.max(0, Math.floor(normalizeNumber(summary.checkpointsPassed, 0)))) },
            { key: 'best-streak', label: 'Beste Serie', value: `x${Math.max(1, Math.floor(normalizeNumber(summary.bestStreak, 1)))}` },
            { key: 'shakeoffs', label: 'Abgeschuettelt', value: String(Math.max(0, Math.floor(normalizeNumber(summary.shakeoffs, 0)))) },
            { key: 'revives', label: 'Rettungen', value: String(Math.max(0, Math.floor(normalizeNumber(summary.revives, 0)))) },
            { key: 'bonus', label: 'Bonus', value: String(Math.max(0, Math.floor(normalizeNumber(summary.bonusScore, 0)))) },
            { key: 'modules', label: 'Module', value: String(Math.max(0, Math.floor(normalizeNumber(summary.completedModules, 0)))) },
            { key: 'wave', label: 'Welle ueberstanden', value: String(Math.max(0, Math.floor(normalizeNumber(summary.lastCompletedWave, 0)))) },
            { key: 'side-routes', label: 'Nebenwege', value: String(Math.max(0, Math.floor(normalizeNumber(summary.sideRoutesCompleted, 0)))) },
            { key: 'flight-objectives', label: 'Flugziele', value: String(Math.max(0, Math.floor(normalizeNumber(summary.flightObjectivesCompleted, 0)))) },
            { key: 'xp', label: 'XP', value: String(Math.max(0, Math.floor(normalizeNumber(summary.xp, 0)))) },
            { key: 'unlocks', label: 'Freischaltungen', value: String(Array.isArray(summary.unlocks) ? summary.unlocks.length : 0) },
            { key: 'persistence', label: 'Speicherung', value: summary.persistence?.pending === true ? 'ausstehend' : 'gespeichert' },
            { key: 'milestones', label: 'Neue Meilensteine', value: String(Array.isArray(summary.newMilestones) ? summary.newMilestones.length : 0) },
        ],
    };
}
...
function buildPostMatchStatsSummary({ recorder, players = [], outcome = null } = {}) {

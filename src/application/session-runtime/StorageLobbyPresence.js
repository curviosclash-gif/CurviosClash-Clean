export const STORAGE_LOBBY_HEARTBEAT_INTERVAL_MS = 4000;

const MEMBER_STALE_AFTER_MS = 15000;
const MEMBER_PRESENCE_LEASE_MS = 45000;
const HOST_PRESENCE_LEASE_MS = 60000;

export function normalizeStorageLobbyMemberSnapshot(member, fallbackRole, now, helpers) {
...
export function normalizeStorageLobbyMembers(members, hostPeerId, now, helpers) {
    const hostId = helpers.normalizeString(hostPeerId, '');
    const normalizedMembers = (Array.isArray(members) ? members : [])
        .map((member) => normalizeStorageLobbyMemberSnapshot(
            member,
...
export function extendStorageLobbyPresenceLease(member, now) {
    return {
        ...member,
        lastSeenAt: now,

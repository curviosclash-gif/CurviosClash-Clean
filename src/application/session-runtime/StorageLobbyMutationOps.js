...
const STORAGE_LOBBY_MAX_PLAYERS = 10;
...
export function joinStorageLobby(bridge, options, helpers) {
    ...
    const persistedSnapshot = bridge._updateActiveSnapshot((existingSnapshot) => {
        ...
        if (!existingLocalMember && existingMembers.length >= STORAGE_LOBBY_MAX_PLAYERS) {
            lobbyFull = true;
            return SNAPSHOT_NOOP;
        }
        return {
    ...
    if (lobbyFull) {
        return bridge._fail(`Lobby ist voll (max. ${STORAGE_LOBBY_MAX_PLAYERS} Spieler).`, 'lobby_full');
    }
    if (hostUnavailable) {
...
export function toggleReadyStorageLobby(bridge, options, helpers) {
...
export function invalidateStorageLobbyReadyForAll(bridge, reason, helpers) {
...
export function publishStorageLobbyHostSettings(bridge, settingsSnapshot, helpers) {
    if (!bridge._activeLobbyCode) return null;
...
export function requestStorageLobbyMatchStart(bridge, options, helpers) {
    if (!bridge._activeLobbyCode) {

// ============================================
// ConfigSections.js - Ausgelagerte Config-Sektionen
// ============================================

import { HUNT_CONFIG } from '../../hunt/HuntConfig.js';
import { createPickupTypeConfigMap } from '../../shared/contracts/PickupRegistryContract.js';

export const CONFIG_SECTIONS = {
    // ---- Allgemein ----
    TICK_RATE: 60,
    TIME_STEP: 1 / 60,
    MAX_UPDATES_PER_FRAME: 5,

    // ---- Arena ----
    ARENA: {
        SIZE: 80,
        WALL_HEIGHT: 30,
        WALL_THICKNESS: 2,
        FLOOR_COLOR: 0x0a0a1a,
        WALL_COLOR: 0x1a1a3a,
        GRID_COLOR: 0x1a1a2e,
        MAP_SCALE: 3,
        CHECKER_LIGHT_COLOR: 0xd9d9d9,
        CHECKER_DARK_COLOR: 0x5a5a5a,
        CHECKER_WORLD_SIZE: 18,
    },

    // ---- Spieler ----
    PLAYER: {
        SPEED: 45,
        TURN_SPEED: 3.4,
        ROLL_SPEED: 3.0,
        BOOST_MULTIPLIER: 2.3,
        BOOST_DURATION: 4.0,
        BOOST_COOLDOWN: 5.0,
        SPAWN_PROTECTION: 1.0,
        HITBOX_RADIUS: 0.8,
        MODEL_SCALE: 1.0,
        NOSE_CAMERA_LOCAL_X: 0,
        NOSE_CAMERA_LOCAL_Y: 0.05,
        NOSE_CAMERA_LOCAL_Z: -1.95,
        START_Y: 10,
        AUTO_ROLL: false,
        AUTO_ROLL_SPEED: 1.5,
        DEFAULT_VEHICLE_ID: 'ship5',
    },

    // ---- Gameplay Options ----
    GAMEPLAY: {
        PLANAR_MODE: false,
        PORTAL_COUNT: 8,
        PLANAR_LEVEL_COUNT: 5,
        PORTAL_BEAMS: false,
        PLANAR_AIM_INPUT_SPEED: 1.5,
        PLANAR_AIM_RETURN_SPEED: 0.6,
    },

    // ---- Hunt Mode ----
    HUNT: {
        ENABLED: true,
        ACTIVE_MODE: HUNT_CONFIG.DEFAULT_MODE,
        RESPAWN_ENABLED: HUNT_CONFIG.DEFAULT_RESPAWN_ENABLED,
        ...HUNT_CONFIG,
    },

    // ---- Trail (Schweifspur) ----
    TRAIL: {
        WIDTH: 0.6,
        UPDATE_INTERVAL: 0.045,
        GAP_CHANCE: 0.02,
        GAP_DURATION: 0.3,
        MAX_SEGMENTS: 5000,
    },

    // ---- Rendering ----
    RENDER: {
        MAX_PIXEL_RATIO: 1.5,
        SHADOW_MAP_SIZE: 512,
    },

    // ---- Powerups ----
    POWERUP: {
        SPAWN_INTERVAL: 3.0,
        MAX_ON_FIELD: 10,
        PICKUP_RADIUS: 2.5,
        SIZE: 1.5,
        ROTATION_SPEED: 2.0,
        BOUNCE_SPEED: 1.5,
        BOUNCE_HEIGHT: 0.5,
        MAX_INVENTORY: 5,
        DURATION: 5.0,

        TYPES: createPickupTypeConfigMap(),
    },

    // ---- Bots ----
    BOT: {
        DEFAULT_DIFFICULTY: 'HARD',
        ACTIVE_DIFFICULTY: 'HARD',
        REACTION_TIME: 0.13,
        LOOK_AHEAD: 13,
        AGGRESSION: 0.52,
        DIFFICULTY_PROFILES: {
            EASY: {
                reactionTime: 0.24,
                lookAhead: 11,
                aggression: 0.36,
                errorRate: 0.24,
                probeSpread: 0.62,
                probeStep: 2.3,
                turnCommitTime: 0.14,
                stuckCheckInterval: 0.45,
                stuckTriggerTime: 1.7,
                minProgressDistance: 0.85,
                minForwardProgress: 0.35,
                recoveryDuration: 0.95,
                recoveryCooldown: 1.9,
                itemUseCooldown: 1.25,
                itemShootCooldown: 0.8,
                targetRefreshInterval: 0.28,
                portalInterest: 0.35,
                portalSeekDistance: 60,
                portalEntryDotMin: 0.22,
                portalIntentThreshold: 0.25,
                portalIntentDuration: 0.9,
                boostChance: 0.0025,
                // Neue Parameter
                probeCount: 7,
                projectileAwareness: 0,
                pursuitEnabled: false,
                pursuitRadius: 0,
                pursuitAimTolerance: 0.95,
                heightBias: 0,
                spacingWeight: 0,
                itemContextWeight: 0.2,
                survivalBias: 0.78,
                boostRiskCeiling: 0.3,
                projectileBoostRiskCeiling: 0.38,
                pursuitRiskCeiling: 0.22,
                pursuitSurvivalCeiling: 0.45,
                emergencyForwardRisk: 0.72,
                emergencyPressure: 0.75,
                emergencyBouncePressure: 1.6,
                collisionPressureRecoveryThreshold: 1.8,
                recoveryBoostRiskCeiling: 0.45,
            },
            NORMAL: {
                reactionTime: 0.14,
                lookAhead: 13,
                aggression: 0.52,
                errorRate: 0.11,
                probeSpread: 0.74,
                probeStep: 1.6,
                turnCommitTime: 0.18,
                stuckCheckInterval: 0.4,
                stuckTriggerTime: 1.2,
                minProgressDistance: 0.9,
                minForwardProgress: 0.45,
                recoveryDuration: 1.3,
                recoveryCooldown: 1.55,
                itemUseCooldown: 0.95,
                itemShootCooldown: 0.62,
                targetRefreshInterval: 0.2,
                portalInterest: 0.56,
                portalSeekDistance: 72,
                portalEntryDotMin: 0.28,
                portalIntentThreshold: 0.2,
                portalIntentDuration: 1.15,
                boostChance: 0.0037,
                // Neue Parameter
                probeCount: 10,
                projectileAwareness: 0.6,
                pursuitEnabled: true,
                pursuitRadius: 35,
                pursuitAimTolerance: 0.85,
                heightBias: 0.15,
                spacingWeight: 0.3,
                itemContextWeight: 0.7,
                survivalBias: 0.7,
                boostRiskCeiling: 0.36,
                projectileBoostRiskCeiling: 0.42,
                pursuitRiskCeiling: 0.3,
                pursuitSurvivalCeiling: 0.56,
                emergencyForwardRisk: 0.78,
                emergencyPressure: 0.82,
                emergencyBouncePressure: 1.9,
                collisionPressureRecoveryThreshold: 2.0,
                recoveryBoostRiskCeiling: 0.5,
            },
            HARD: {
                reactionTime: 0.08,
                lookAhead: 16,
                aggression: 0.68,
                errorRate: 0.04,
                probeSpread: 0.9,
                probeStep: 1.4,
                turnCommitTime: 0.24,
                stuckCheckInterval: 0.35,
                stuckTriggerTime: 1.0,
                minProgressDistance: 1.0,
                minForwardProgress: 0.5,
                recoveryDuration: 1.25,
                recoveryCooldown: 1.2,
                itemUseCooldown: 0.78,
                itemShootCooldown: 0.48,
                targetRefreshInterval: 0.12,
                portalInterest: 0.74,
                portalSeekDistance: 84,
                portalEntryDotMin: 0.35,
                portalIntentThreshold: 0.14,
                portalIntentDuration: 1.35,
                boostChance: 0.0055,
                // Neue Parameter
                probeCount: 12,
                projectileAwareness: 0.95,
                pursuitEnabled: true,
                pursuitRadius: 44,
                pursuitAimTolerance: 0.75,
                heightBias: 0.25,
                spacingWeight: 0.5,
                itemContextWeight: 1.0,
                survivalBias: 0.62,
                boostRiskCeiling: 0.4,
                projectileBoostRiskCeiling: 0.46,
                pursuitRiskCeiling: 0.36,
                pursuitSurvivalCeiling: 0.62,
                emergencyForwardRisk: 0.82,
                emergencyPressure: 0.88,
                emergencyBouncePressure: 2.1,
                collisionPressureRecoveryThreshold: 2.2,
                recoveryBoostRiskCeiling: 0.54,
            },
            PPO_V2: {
                reactionTime: 0.1,
                lookAhead: 15,
                aggression: 0.72,
                errorRate: 0.0,
                probeSpread: 0.9,
                probeStep: 1.4,
                turnCommitTime: 0.24,
                stuckCheckInterval: 0.35,
                stuckTriggerTime: 1.0,
                minProgressDistance: 1.0,
                minForwardProgress: 0.5,
                recoveryDuration: 1.25,
                recoveryCooldown: 1.2,
                itemUseCooldown: 0.78,
                itemShootCooldown: 0.48,
                targetRefreshInterval: 0.12,
                portalInterest: 0.74,
                portalSeekDistance: 84,
                portalEntryDotMin: 0.35,
                portalIntentThreshold: 0.14,
                portalIntentDuration: 1.35,
                boostChance: 0.0055,
                probeCount: 12,
                projectileAwareness: 0.95,
                pursuitEnabled: true,
                pursuitRadius: 44,
                pursuitAimTolerance: 0.75,
                heightBias: 0.25,
                spacingWeight: 0.5,
                itemContextWeight: 1.0,
                survivalBias: 0.62,
                boostRiskCeiling: 0.4,
                projectileBoostRiskCeiling: 0.46,
                pursuitRiskCeiling: 0.36,
                pursuitSurvivalCeiling: 0.62,
                emergencyForwardRisk: 0.82,
                emergencyPressure: 0.88,
                emergencyBouncePressure: 2.1,
                collisionPressureRecoveryThreshold: 2.2,
                recoveryBoostRiskCeiling: 0.54,
            },
        },
    },

    // ---- Projektile (Item-Schuss) ----
    PROJECTILE: {
        SPEED: 45,
        RADIUS: 0.7,
        LIFE_TIME: 3.0,
        MAX_DISTANCE: 140,
        COOLDOWN: 1.25,
        PLANAR_AIM_MAX_ANGLE_DEG: 18,
    },

    // ---- Portale ----
    PORTAL: {
        RADIUS: 4.0,
        COOLDOWN: 1.2,
        RING_SIZE: 4.0,
        ROTATION_SPEED: 2.0,
        MIN_PAIR_DISTANCE: 15,
        MIN_PAIR_DISTANCE_PLANAR: 4,
    },

    // ---- Homing (Zielsuchend) ----
    HOMING: {
        LOCK_ON_ANGLE: 11,       // Grad – Lock-On-Kegel
        TURN_RATE: 3.0,          // Wie schnell Rakete zum Ziel lenkt (Rad/s)
        MAX_LOCK_RANGE: 100,     // Maximale Lock-On-Entfernung
        LEAD_TIME_MAX: 0.35,     // Sekunden Vorhalt auf Zielgeschwindigkeit
        TURN_DOT_BLEND: 0.35,    // 0=konstante Turn-Rate, 1=stark winkelabhaengig
    },

    // ---- Farben ----
    COLORS: {
        PLAYER_1: 0x00aaff,
        PLAYER_2: 0xff8800,
        BOT_COLORS: [0xff4444, 0x44ff44, 0xffff44, 0xff44ff, 0x44ffff],
        BACKGROUND: 0x080812,
        AMBIENT_LIGHT: 0x334466,
        POINT_LIGHT: 0xffffff,
    },

    // ---- Kamera ----
    CAMERA: {
        FOV: 75,
        NEAR: 0.1,
        FAR: 200,
        CINEMATIC_ENABLED: true,
        FOLLOW_DISTANCE: 9,
        FOLLOW_HEIGHT: 5,
        LOOK_AHEAD: 7,
        SMOOTHING: 0.05,
        MODES: ['THIRD_PERSON', 'FIRST_PERSON', 'TOP_DOWN'],
        FIRST_PERSON_LOCK_TO_NOSE: true,
        FIRST_PERSON_NOSE_CLEARANCE: 0.3,
        FIRST_PERSON_OFFSET: 4.0, // Kamera vor dem Flugzeug
        FIRST_PERSON_BOOST_OFFSET: 0.85, // waehrend Boost naeher an die Nase
        FIRST_PERSON_BOOST_BLEND_SPEED: 8.5,
        COLLISION_RADIUS: 0.45,
        COLLISION_BACKOFF: 0.04,
        COLLISION_STEPS: 8,
    },


    // ---- Tastenbelegung ----
    KEYS: {
        PLAYER_1: {
            UP: 'KeyW',
            DOWN: 'KeyS',
            LEFT: 'KeyA',
            RIGHT: 'KeyD',
            ROLL_LEFT: 'KeyQ',
            ROLL_RIGHT: 'KeyE',
            BOOST: 'ShiftLeft',
            SHOOT: 'KeyF',
            SHOOT_MG: 'KeyX',
            NEXT_ITEM: 'KeyR',
            USE_ITEM: 'KeyG',
            CAMERA: 'KeyC',
        },
        PLAYER_2: {
            UP: 'Numpad8',
            DOWN: 'Numpad5',
            LEFT: 'Numpad4',
            RIGHT: 'Numpad6',
            ROLL_LEFT: 'Numpad7',
            ROLL_RIGHT: 'Numpad9',
            BOOST: 'ArrowRight',
            SHOOT: 'ArrowUp',
            SHOOT_MG: 'ArrowDown',
            NEXT_ITEM: 'ArrowLeft',
            USE_ITEM: 'Quote',
            CAMERA: 'NumpadDivide',
        },
        GLOBAL: {
            CINEMATIC_TOGGLE: 'F8',
            RECORDING_TOGGLE: 'F9',
        }
    }
};

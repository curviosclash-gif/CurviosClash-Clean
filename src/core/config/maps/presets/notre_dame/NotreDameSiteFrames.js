...
// Which drawn machine each supplemental box braces. A box only makes sense while its GLB is on
// the map: a variant that drops the machine has to drop its collision with it, or the box stays
// as an invisible wall. Filled by frame() as the list below is built.
const SITE_FRAME_MODEL_ID_BY_FRAME_ID = new Map();

/**
 * A standing part, stated as the box it occupies.
 *
 * @param {string} modelId the GLB model this box braces
 ...
 */
function frame(modelId, id, pos, size) {
    const frameId = `nd-site-${id}`;
    SITE_FRAME_MODEL_ID_BY_FRAME_ID.set(frameId, modelId);
    return { id: frameId, pos, size, compileWithGlb: true };
}

function latticeFrames(modelId, id, pos, legOffset, height) {
    const [x, y, z] = pos;
    const legSize = 0.45;
...
export const NOTRE_DAME_SITE_FRAMES = [
    ...
    ...latticeFrames('notre-dame-tower-crane', 'crane-mast', [17.0, GROUND + 37.8, -90.0], 2.1, 75.6),
    // The base is round. This square is inscribed in it, so its corners never reach into air.
    frame('notre-dame-tower-crane', 'crane-base', [17.0, GROUND + 1.4, -90.0], [6.3, 2.8, 6.3]),
    ...
    ...latticeFrames('notre-dame-stone-hoist', 'stone-hoist-west-tower', [-28.8, GROUND + 15.0, -55.0], 1.26, 30.0),
    ...latticeFrames('notre-dame-stone-hoist', 'stone-hoist-east-tower', [8.8, GROUND + 15.0, -55.0], 1.26, 30.0),
    frame('notre-dame-stone-hoist', 'stone-hoist-beam', [-10.0, GROUND + 30.8, -55.0], [39.2, 1.68, 1.96]),
    ...
    ...latticeFrames('notre-dame-scaffold-lift', 'scaffold-mast', [-40.0, GROUND + 23.8, 47.8], 1.68, 47.6),
    frame('notre-dame-scaffold-lift', 'scaffold-foot', [-40.0, GROUND + 0.56, 47.8], [8.4, 1.12, 8.4]),
    ...[13.6, 21.2, 28.7, 36.3, 43.8, 51.4].map((y, level) => (
        frame('notre-dame-scaffold-lift', `scaffold-deck-${level}`, [-40.0, y, 41.6], [7.3, 0.4, 7.3])
    )),
    ...
    frame('notre-dame-hoarding', 'hoarding-south-post', [-149.9, GROUND + 12.6, -21.84], [1.96, 25.2, 1.4]),
    frame('notre-dame-hoarding', 'hoarding-north-post', [-149.9, GROUND + 12.6, 21.84], [1.96, 25.2, 1.4]),
    frame('notre-dame-hoarding', 'hoarding-head', [-149.9, GROUND + 25.76, 0.0], [1.68, 1.4, 44.8]),
    ...
    frame('notre-dame-fleche-hoist', 'fleche-cradle', [130.0, GROUND + 1.68, 0.0], [14.0, 3.36, 14.0]),
    frame('notre-dame-fleche-hoist', 'fleche-head', [130.0, GROUND + 42.56, 0.0], [20.16, 1.4, 20.16]),
];

export const NOTRE_DAME_SITE_FRAME_MODEL_ID_BY_FRAME_ID = SITE_FRAME_MODEL_ID_BY_FRAME_ID;

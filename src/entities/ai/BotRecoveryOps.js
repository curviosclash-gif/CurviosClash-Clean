...
function selectRecoveryManeuver(bot, player, arena, allPlayers) {
    ...
    for (let i = 0; i < candidates.length; i++) {
        ...
        for (let j = 0; j < sampleDistances.length; j++) {
            ...
            const wallHit = (arena.checkBotCollisionFast || arena.checkCollisionFast).call(arena, bot._tmpVec2, player.hitboxRadius * 1.6);
...
function shouldBoostRecovery(bot, player, arena, allPlayers) {
    ...
    for (let i = 0; i < checks.length; i++) {
        ...
        if ((arena.checkBotCollisionFast || arena.checkCollisionFast).call(arena, bot._tmpVec2, player.hitboxRadius * 1.6) || bot.checkTrailHit(bot._tmpVec2, player, allPlayers)) {
            return false;
        }
    }
    return true;

...
function createGeneratedLocalMapsModuleContent(payload) {
    return `// Auto-generated by the editor disk-save API. Do not edit manually.\n` +
        `export const GENERATED_LOCAL_MAPS = ${JSON.stringify(payload, null, 2)};\n\n` +
        `export default GENERATED_LOCAL_MAPS;\n`;
}

export function writeFilesAtomically(files) {
    const token = `${process.pid}-${randomUUID()}`;
    const entries = files.map(({ filePath, content }) => ({
        filePath: path.resolve(filePath),
        content,
        tempPath: `${path.resolve(filePath)}.${token}.tmp`,
        backupPath: `${path.resolve(filePath)}.${token}.bak`,
        hadOriginal: false,
        committed: false,
    }));
    ...
    try {
        for (const entry of entries) {
            mkdirSync(path.dirname(entry.filePath), { recursive: true });
            writeFileSync(entry.tempPath, entry.content, 'utf-8');
        }
        for (const entry of entries) {
            entry.hadOriginal = existsSync(entry.filePath);
            if (entry.hadOriginal) renameSync(entry.filePath, entry.backupPath);
        }
        for (const entry of entries) {
        ...
        for (const entry of entries) {
            ...
            try {
                ...
                console.warn(`[editor-disk-save] Backup cleanup failed for ${entry.backupPath}:`, cleanupError);
            }
        }
    } catch (error) {
        ...
        if (rollbackError) {
            throw new AggregateError([error, rollbackError], 'Map export failed and rollback was incomplete.');
        }
        throw error;
    }
}
...
function openMapsFolder() {
    ...
    const command = process.platform === 'win32'
        ? ['explorer.exe', [EDITOR_MAP_DIR]]
        : process.platform === 'darwin'
            ? ['open', [EDITOR_MAP_DIR]]
            : ['xdg-open', [EDITOR_MAP_DIR]];
    return new Promise((resolve, reject) => {
        const child = spawn(command[0], command[1], { detached: true, stdio: 'ignore' });
        child.once('error', reject);
        child.once('spawn', () => {
            child.unref();
            resolve({ folderPath: path.relative(__dirname, EDITOR_MAP_DIR).replace(/\\/g, '/') });
        });
    });
}
...
export function editorDiskSaveApiPlugin() {
    ...
    const mapListRoutePath = EDITOR_API_ROUTES.LIST_MAPS_DISK;
    const mapFolderRoutePath = EDITOR_API_ROUTES.OPEN_MAPS_FOLDER;
    ...
    const registerMiddleware = (middlewares, { isPreviewServer = false } = {}) => {
        middlewares.use(async (req, res, next) => {
            ...
            const isMapList = req.method === 'GET' && reqPath === mapListRoutePath;
            const isMapFolderOpen = req.method === 'POST' && reqPath === mapFolderRoutePath;
            ...
            if (!isMapSave && !isMapList && !isMapFolderOpen && !isVehicleSave && !isVehicleList && !isVehicleGet && !isVehicleRename && !isVehicleDelete && !isVideoSave) {
                next();
                return;
            }

            const isLocalMutation = isMapSave || isMapFolderOpen || isVehicleSave || isVehicleRename || isVehicleDelete || isVideoSave;
            ...
            try {
                ...
                const saveAsCopy = body?.saveAsCopy === true;
                ...
                const result = isMapSave
                    ? saveEditorMapToDisk({ jsonText, mapName, editorDocument, saveAsCopy })
                    : isVehicleSave

...
test('heuristic improvement loop separates coarse training from rotated holdout validation', () => {
    ...
    assert.match(source, /verifyCurrentProfiles\(parseFreshAuditSeeds\(process\.argv\[3\]\), false, true, profile \? \[profile\] : PROFILES\)/);
    assert.match(source, /new Set\(seeds\)\.size !== seeds\.length/);

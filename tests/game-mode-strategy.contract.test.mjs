...
test('Fight fans receive 5/4/3 percent and every other base spawn chance shrinks proportionally', () => {
    ...
    const weightedTypes = new Set(['FOG', 'FAN_3', 'FAN_4', 'FAN_5', 'FLAMETHROWER', 'LIGHTNING']);
    ...
    // The fan targets were tuned before the flamethrower and the lightning existed. A new item in the
    // pool takes its share from everyone, so every older target shrinks by the same factor.
    const dilution = (totalNonRocketWeight - weights.FLAMETHROWER - weights.LIGHTNING) / totalNonRocketWeight;

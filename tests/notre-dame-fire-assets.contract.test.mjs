...
const PARTS = Object.freeze({
    '02_nave_burnt': { centerX: -24.68, floorY: -0.8, spanY: 34.1 },
    '03_transept_burnt': { centerX: 12.25, floorY: -0.8, spanY: 41.9 },
    // The intact roof part is 82.1 m tall because the spire is in it. This one is not.
    '06_roof_burnt': { centerX: 3.79, floorY: 14.06, spanY: 21.5 },
    '20_fleche_debris': { centerX: 12.25, floorY: -0.23, spanY: 9.8 },
});
...
/** Bounding box from the POSITION accessors' mandatory min/max. Static parts bake every
 *  transform into their vertices when they are merged, so this is already world space. */
function boundingBox(document, { collidableOnly = false } = {}) {
    const low = [Infinity, Infinity, Infinity];
    const high = [-Infinity, -Infinity, -Infinity];
    const meshIsCollidable = new Map();
    for (const node of document.nodes || []) {
        if (node.mesh === undefined) continue;
        meshIsCollidable.set(node.mesh, !String(node.name || '').toLowerCase().includes('_nocol'));
    }
    for (const [index, mesh] of (document.meshes || []).entries()) {
        if (collidableOnly && meshIsCollidable.get(index) !== true) continue;
...
function nodeNames(fileStem) {
    return (readGlbJson(fileStem).nodes || []).map((node) => String(node.name || '')).filter(Boolean);
}
...
test('the preset places every burnt part at the measurement its file reports', () => {
    ...
    for (const [fileStem, expected] of Object.entries(PARTS)) {
        ...
        assert.equal(model.position[2], 0, `${fileStem} sits on the building axis`);
    }
});
...
test('the debris cone is an obstacle and the broken rims are not', () => {
    ...
    const debris = nodeNames('20_fleche_debris');
    assert.ok(
